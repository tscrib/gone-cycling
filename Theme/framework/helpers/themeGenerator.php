...
//				$text = sprintf(__('Category Archive for: ‘%s’','striking_front'),single_cat_title('',false));
	...
	function portfolio_featured_image($layout=''){
		...
		$image_src_array = the_post_thumbnail(get_post_thumbnail_id(),'full', true);
		...
		// $image_src = get_image_src($image_src_array[0]).'?resize='.$width.'%2C'.$height;
		// $image_src = get_image_src($image_src_array[0]);
...
HTML;
				...
				// $image_src = $post['image'].'?h='.$height;
				// $image_src = $post['image'];

				echo '<div class="feature_box_image"><a href="'.$post['link'].'"><img src="'.$image_src.'" alt="'.$post['title'].'" /></a><</div>';
			}
			...
			switch(get_post_meta($image['id'], '_anything_type', true)){
				case 'sidebar':
					...
					if($image['link'] != ''){
						...
						// echo '<a href="'.$image['link'].'"><img '.get_image_src($image['src']).'?h='.$height.'" alt="" /></a>';
						// echo '<a href="'.$image['link'].'"><img '.get_image_src($image['src']).'" alt="" /></a>';

					}else{
						echo '<img src="' . THEME_INCLUDES.'/timthumb.php?src='.get_image_src($image['src']).'&amp;h='.$height.'&amp;w=660&amp;zc=1" alt="" />';
						// echo '<img '.get_image_src($image['src']).'?h='.$height.'" alt="" />';
						// echo '<img '.get_image_src($image['src']).'" alt="" />';
