...
//	Modify image call to fix incompatibility with NextGEN gallery plugin
//	Author: 	Tim Scribner
//
//	Log: 	April 2013 - Replaced wp_get_attachment_image_src call
//			May 2013 - Replaced get_post_meta calls
//
//				$image_src_array = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full', 'false');
				$image_src_array = Display_Featured_Image_URL(get_post_thumbnail_id(), 'full');
				$image_src = THEME_INCLUDES.'/timthumb.php?src='.get_image_src($image_src_array[0]).'&amp;h='.$height.'&amp;w=960&amp;zc=1';
				// $image_src = get_image_src($image_src_array[0]).'&amp;h='.$height.'&amp;w=960&amp;zc=1';
				$featured_content = '<div class="feature_box_image"><img src="'.$image_src.'" title="'.TS_get_the_alt(get_post_thumbnail_id()).'"  alt="'.TS_get_the_alt(get_post_thumbnail_id()).'" /></div>';
...
//	Modify image call to fix compatibility with NextGEN gallery plugin
//	Author:	Tim Scribner
//	Date:	7 May 2013
//				$image_src_array = wp_get_attachment_image_src(get_post_thumbnail_id(),'full', true);
				$image_src_array = Display_Featured_Image_URL(get_post_thumbnail_id(), 'full');
				$image_src = THEME_INCLUDES.'/timthumb.php?src='.get_image_src($image_src_array[0]).'&amp;h=80&amp;w=192&amp;zc=1';				
				// $image_src = get_image_src($image_src_array[0]).'&amp;h=80&amp;w=192&amp;zc=1';
